...
def test__migrate() -> None:
    ...
    connection.execute.return_value.fetchone.side_effect = [None, None, None, None]
    ...
    connection.execute.return_value.fetchone.side_effect = [{"?column?": 1}, {"?column?": 1}, {"?column?": 1}, {"?column?": 1}]
    result = tested._migrate(connection)
    assert result is None
    exp_calls = [
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (1,)),
