...
def test_class() -> None:
    tested = SessionUser
    fields = ["user_id", "email", "name", "is_admin", "reminder"]
...
def test_to_dict() -> None:
    tests: list[tuple[SessionUser, dict[str, int | str | bool]]] = [
        (
            SessionUser(user_id=7, email="jane@example.com", name="Jane Doe", is_admin=True, reminder=True),
            {"user_id": 7, "email": "jane@example.com", "name": "Jane Doe", "is_admin": True, "reminder": True},
        ),
        (
            SessionUser(user_id=3, email="john@example.com", name="", is_admin=False),
            {"user_id": 3, "email": "john@example.com", "name": "", "is_admin": False, "reminder": False},
        ),
    ]
