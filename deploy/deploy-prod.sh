...
# Runs on the server from the git clone in /opt/usage, as its owner. It
# fast-forwards the checked-out branch itself (deploy-from-local.sh triggers it
# from the laptop), so production always runs the last pushed commit.
#
...
# The body is one main() invoked on the last line: bash reads a script
# incrementally, and the fast-forward in step 1 may replace this very file.
main() {
    # Run as the clone's owner or not at all, checked before any git command: git
    # calls a clone owned by someone else "dubious" and fails in ways that look
    # like a missing clone. Root once half-ran the Our Stories deploy that way.
    REPO_OWNER="$(stat -c %U .)"
    ...
    # The version number counts the day's commits: a shallow clone counts wrong.
    if [ "$(git rev-parse --is-shallow-repository)" = "true" ]; then
        echo "❌ Shallow clone; the version number counts commits:  git -C $(pwd) fetch --unshallow"
        exit 1
    fi
    BRANCH=$(git rev-parse --abbrev-ref HEAD)
    if [ "$BRANCH" = "HEAD" ]; then
        echo "❌ Detached HEAD; check out the branch to deploy:  git -C $(pwd) checkout main"
        exit 1
    fi
    echo "  branch: $BRANCH"
    # The caller names the branch it verified as pushed (deploy-from-local.sh):
    # deploying a different one would look green while shipping other code.
    ...
    # up to date" and the build takes the edit with it. Ignored files — .env, the CSV exports —
    ...
    echo "  deploying: $BRANCH @ $(git log -1 --format='%h %s')"
    echo ""
