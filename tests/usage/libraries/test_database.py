...
def test__migrate() -> None:
    ...
    connection.execute.return_value.fetchone.side_effect = [None] * 17
    ...
    exp_calls = [
        ...
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (3,)),
        call.execute().fetchone(),
        ...
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (13,)),
        call.execute().fetchone(),
        ...
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (14,)),
        call.execute().fetchone(),
        call.execute(
            "CREATE TABLE IF NOT EXISTS water_feeds ("
            "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
        ...
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (15,)),
        call.execute().fetchone(),
        ...
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (16,)),
        call.execute().fetchone(),
        ...
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (17,)),
        call.execute().fetchone(),
        call.execute(
            "CREATE TABLE IF NOT EXISTS water_alerts ("
            "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
    ...
    connection.execute.return_value.fetchone.side_effect = [{"?column?": 1}] * 17
    result = tested._migrate(connection)
    assert result is None
    exp_calls = [
        call.execute("SELECT 1 FROM schema_migrations WHERE version = %s", (1,)),
