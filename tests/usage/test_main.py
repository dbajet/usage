...
def test_create() -> None:
    ...
    assert database_class.mock_calls == [call(settings)]
    assert static_page_class.mock_calls == [call(Path("/media/APPLICATIONS/git_dbajet/usage/app/usage/static"))]
    # the process's one window per API reaches the router and the sync alike
    exp_calls = [call(database, settings, tested._enphase_limiter, tested._switchbot_limiter)]
    assert api_router_class.mock_calls == exp_calls
    ...
    switch_bot_sync_command = MagicMock()
    with (
        ...
        patch("usage.main.SwitchBotSyncCommand") as switch_bot_sync_command_class,
    ):
        email_sender_class.side_effect = [email_sender, email_sender, email_sender]
        ...
        switch_bot_sync_command_class.side_effect = [switch_bot_sync_command]
        asyncio.run(run_lifespan(result))
    assert database.mock_calls == [call.initialize()]
    assert email_sender_class.mock_calls == [call(settings), call(settings), call(settings)]
    ...
    # The thermometers it pulls can breach an alert range like any other, so it
    # takes the email sender the pushed ones already go through.
    exp_calls = [call(database, settings, email_sender, tested._switchbot_limiter)]
    assert switch_bot_sync_command_class.mock_calls == exp_calls
    ...
    assert switch_bot_sync_command.mock_calls == [call.start()]
